...
Versuch die Zuständigkeiten zu scrapen - Hat funktioniert!!

```{r}
zustandbran <- data.frame()

for (i in branden) {
  if (i > 1) {
    Sys.sleep(3 + runif(5))
  }
  urlbran <- paste0("https://www.hartziv.org/jobcenter/brandenburg/", i, ".html")
  quelbran <- read_html(urlbran)
  print(urlbran)
  nodesbran <- html_nodes(quelbran, xpath = '//*[contains(concat( " ", @class, " " ), concat( " ", "striped_bordered", " " ))]')
  nodesbran <- html_table(nodesbran)
  zustandbran <- append(zustandbran, nodesbran)
}

zustandbran
```

Neuer Versuch mit html_tables --> Ignorieren!!!!
...
for (i in branden) {
  ...
  urlbran <- paste0("https://www.hartziv.org/jobcenter/brandenburg/", i, ".html")
  quelbran <- xml2::read_html(url)
  print(urlbran)
  nodesbayern <- rvest::html_table(webpage)[[2]] %>% 
  tibble::as_tibble(.name_repair = "unique")
  zustandbrand <- append(zustandbrand, nodesbran)
}

zustandbrand
```
Nur eine Seite scrapen: Tim Versuch

```{r}

url <- "https://www.hartziv.org/jobcenter/brandenburg/jobcenter-barnim.html"
p <- read_html(url)
table3 <- p %>% html_nodes("#content > div > div > div.nv-single-page-wrap.col > div.nv-content-wrap.entry-content > table") %>% html_table(trim=T)
